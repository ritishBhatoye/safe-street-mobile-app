...
export const ReportCard: React.FC<ReportCardProps> = ({ item, index, onPress }) => {
  ...
  return (
    ...
    >
      <TouchableOpacity
        className="mb-4 rounded-2xl shadow-md p-4 border border-gray-100 bg-white dark:bg-gray-800 shadow-md'"
        activeOpacity={0.8}
      ...
      >
        ...
        <View className="flex-row justify-between items-start mb-3">
          <View className="flex-row items-center flex-1">
            ...
            <View className="flex-1">
              ...
              <View className="flex-row items-center">
                ...
                <Text className="text-xs text-gray-600 dark:text-gray-400 font-dm-sans ml-1 ">
          ...
          <View className={`px-3 py-1 rounded-full ${getSeverityColor(item.severity)}`}>
            <Text className="text-xs font-bold text-white capitalize">{item.severity}</Text>
          </View>
        </View>

        {/* Description */}
        <Text
          className="text-sm text-gray-700 dark:text-gray-300 font-dm-sans mb-3"
          numberOfLines={2}
        >
          {item.description}
        </Text>
        ...
        <View className="flex-row justify-between items-center pt-3 border-t border-gray-100">
          <View className="flex-row items-center flex-1">
            ...
            <Text
              className="text-xs text-gray-700 dark:text-gray-300 ml-1 flex-1"
              numberOfLines={1}
            >
          ...
          <View className={`px-3 py-1 rounded-full ${getStatusColor(item.status)}`}>
            <Text className="text-xs font-medium capitalize">{item.status.replace("-", " ")}</Text>
          </View>
        </View>
      </TouchableOpacity>
    </Animated.View>
